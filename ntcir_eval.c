...
- updated 20200527: introduces NCUrb,IE (inverse effort, using the RBU parameter p as in iRBU)
  Precision@l now shown as Prec@l instead of P@l
  Cutoff-based NCUrb,* measures are also output
  No longer outputs NCUgu,* for compute (gcompute never output NCUgu,* since these measures assume discrete relevance levels)
- updated 20200515: now computes iRBU@ and iRBU by default
- updated August 2019: added iRBU in gcompute
******************************************************************************/
...
- Does not support graded-uniform NCU (because graded uniform assumes discrete relevance levels)
...
20190831: now computes iRBU just like compute.
...
/* ditto for system ranked list */
  ...
  double *RBPisum; /* 2020 for normalisation */
  ...
  double *EBRi; /* 2020 for normalisation */
  double RBUp = DEFAULT_RBU; /* RBU p parameter */
  double *iRBU;/* intentwise RBU, where RBU is from SIGIR18 */
  double *iRBUi; /* 2020 for normalisation */
 ...
 /* 2020 for normalisation */
  EBRi = ( double *)calloc( maxlen + 1, sizeof( double ) );
  iRBUi = ( double *)calloc( maxlen + 1, sizeof( double ) );
  RBPisum = ( double *)calloc( maxlen + 1, sizeof( double ) );

  if( compute_gap ){ /* added IF on 20190831 to be consistent with compute */
  ...
  for( i = 1; i <= jrelnum; i++ ){
    ...
    iRBUi[ i ] = iRBUi[ i - 1 ] +
      pow( RBUp, i ) * ERRidsat[ i - 1 ] * ( doc->val/( hgain+1 ) );
    EBRi[ i ] = EBRi[ i - 1 ] +
      ERRidsat[ i - 1 ] * ( doc->val/( hgain+1 ) );
    /* BR[ i ] == 1 for an ideal list for i<=R */
    
    ERRidsat[ i ] = ERRidsat[ i - 1 ] * ( 1 - doc->val/( hgain+1 ) );

    /* 2020 for normalisation */
    RBPisum[ i ] = RBPisum[ i - 1 ] + doc->val * pow( RBPp, i - 1 );
  ...
  for( i = jrelnum + 1; i <= maxlen; i++ ){
    ...
    /* 2020 for normalisation */
    EBRi[ i ] = EBRi[ jrelnum ];
    iRBUi[ i ] = iRBUi[ jrelnum ];
    RBPisum[ i ] = RBPisum[ jrelnum ];
  }
  ...
  for( i = 1; i <= syslen; i++ ){
    ...
    RBPsum[ i ] = RBPsum[ i - 1 ] + Gs[ i ] * pow( RBPp, i - 1 );
    ...
    */
    ...
    iRBU[ i ] = iRBU[ i - 1 ] + pow( RBUp, i ) * ERRdsat[ i - 1 ]*Gs[ i ]/( hgain+1 );
  ...
  for( i = 1; i <= syslen; i++ ){
    if( Gs[ i ] > 0 ){
      ...
      sum3 += pow( RBUp, i ) *  pow( gamma, count[ i ]-1 ); /* 20200527 */
    }
  }
  printf( "%s NCUrb,P=             %.4f\n", outstr, sum/denom );
  printf( "%s NCUrb,BR=            %.4f\n", outstr, sum2/denom );
  printf( "%s NCUrb,IE=            %.4f\n", outstr, sum3/denom ); /* 20200527 */
  ...
  for( i = 0; i < cutoff_num; i++ ){
    ...
    for( j = 1; j <= minlen; j++ ){
      if( Gs[ j ] > 0 ){
        sum += ( count[ j ]/j ) * pow( gamma, count[ j ]-1 );
        sum2 += BR[ j ] * pow( gamma, count[ j ]-1 );
        sum3 += pow( RBUp, j ) * pow( gamma, count[ j ]-1 ); 
      }
    }

    /* divide by min(cutoff, jrelnum ) */
    if( cutoff[ i ] < jrelnum ){
      printf( "%s NCUrb,P@%04ld=        %.4f\n", outstr, cutoff[ i ], sum/cutoff[ i ] );
      printf( "%s NCUrb,BR@%04ld=       %.4f\n", outstr, cutoff[ i ], sum2/cutoff[ i ] );
      printf( "%s NCUrb,IE@%04ld=       %.4f\n", outstr, cutoff[ i ], sum3/cutoff[ i ] );
    }
    else{
      printf( "%s NCUrb,P@%04ld=        %.4f\n", outstr, cutoff[ i ], sum/jrelnum );
      printf( "%s NCUrb,BR@%04ld=       %.4f\n", outstr, cutoff[ i ], sum2/jrelnum );
      printf( "%s NCUrb,IE@%04ld=       %.4f\n", outstr, cutoff[ i ], sum3/jrelnum );
    }
    ...
    if( cutoff[ i ] <= maxlen ){
      ...
      /* 2020 introduced nEBR, niRBU, nRBP */
      ...
      printf( "%s nRBP@%04ld=           %.4f\n",
              outstr, cutoff[ i ], RBPsum[ cutoff[ i ] ]/RBPisum[ cutoff[ i ] ]);
      ...
      printf( "%s nEBR@%04ld=           %.4f\n",
              outstr, cutoff[ i ], EBR[ cutoff[ i ] ]/EBRi[ cutoff[ i ] ]);
      printf( "%s iRBU@%04ld=           %.4f\n",
              outstr, cutoff[ i ], iRBU[ cutoff[ i ] ] );
      printf( "%s niRBU@%04ld=          %.4f\n",
              outstr, cutoff[ i ], iRBU[ cutoff[ i ] ]/iRBUi[ cutoff[ i ] ]);
	...
	printf( "%s Hit@%04ld=            %.4f\n",
      ...
      /* 20181003 note that hgain is obtained PER TOPIC */
      printf( "%s nRBP@%04ld=           %.4f\n",
              outstr, cutoff[ i ], RBPsum[ maxlen ]/RBPisum[ maxlen ]);
      printf( "%s ERR@%04ld=            %.4f\n",
      ...
      printf( "%s nEBR@%04ld=           %.4f\n",
              outstr, cutoff[ i ], EBR[ maxlen ]/EBRi[ maxlen ] );
      printf( "%s iRBU@%04ld=           %.4f\n",
              outstr, cutoff[ i ], iRBU[ maxlen ] );
      printf( "%s niRBU@%04ld=          %.4f\n",
              outstr, cutoff[ i ], iRBU[ maxlen ]/iRBUi[ maxlen ] );
...
#ifdef OUTERR
      fprintf( stderr, "strtok for gv failed\n" );
#endif
      return( -1 );
    }
...
20200527 No longer outputs NCUgu,* just like gcompute.
(These measures assume discrete relevance levels.)
...
/* ditto for system ranked list */
  ...
  for( rlevel = maxrl; rlevel >= 1; rlevel-- ){
	...
	GAPisum[ i ] = GAPisum[ i - 1 ] + rlevel * ( rlevel + 1 );
      ...
      iRBUi[ i ] = iRBUi[ i - 1 ] + 
	pow( RBUp, i ) * ERRidsat[ i - 1 ] * ( gv[ rlevel ]/( gv[ maxrl ]+1 ) );
      EBRi[ i ] = EBRi[ i - 1 ] +
	ERRidsat[ i - 1 ] * ( gv[ rlevel ]/( gv[ maxrl ]+1 ) );
      /* BR[ i ] == 1 for an ideal list for i<=R */

      ERRidsat[ i ] = ERRidsat[ i - 1 ] * ( 1 - gv[ rlevel ]/( gv[ maxrl ]+1 ) );

      /* 2020 for normalisation */
      RBPisum[ i ] = RBPisum[ i - 1 ] + gv[ rlevel ] * pow( RBPp, i - 1 );
  ...
  for( i = 1; i<= syslen; i++ ){
    ...
    */
    ...
    iRBU[ i ] = iRBU[ i - 1 ] + pow( RBUp, i ) * ERRdsat[ i - 1 ]*Gs[ i ]/( gv[ maxrl ]+1 );
  ...
  */
  ...
  /* note that the denom will be resued below for cutoff-based NCUbr,* */
     ...
     char **av;
{
  ...
  fprintf( stderr, "\t%s <positive value <=1 >: persistence for iRBU and NCUrb,IE (default: %.2f)\n",
	   OPTSTR_RBUP, DEFAULT_RBU );
  fprintf( stderr,
	   "\t%s <document rank[,document rank,...]>: cutoffs for Prec@n, Hit@n, nDCG@n... (default: %d)\n",
